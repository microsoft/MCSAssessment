function Build-MCSMediumImpactSlide {
    Param($Presentation,$CustomerName,$MediumRecommendation)
    ...
    $Counter = 1
    $RecomNumber = 1
    foreach ($Medium in $MediumRecommendation) {
        $RecomName = $Medium.Recommendation
        $Implication = $Medium.Implication
        Write-Debug ((get-date -Format 'yyyy-MM-dd HH:mm:ss') + ' - Editing Slide 11 - Adding Recommendation: ' + $RecomName)
        if ($Counter -lt 9) {
            ...
            Write-Debug ((get-date -Format 'yyyy-MM-dd HH:mm:ss') + ' - Editing Slide 11 - Adding new Slide..')
            $Counter = 1
            ...
            Write-Debug ((get-date -Format 'yyyy-MM-dd HH:mm:ss') + ' - Editing Slide 11 - Cleaning table of new slide..')
            $rowTemp = 2
            while ($rowTemp -lt 10) {
                $cell = 1
                while ($cell -lt 5) {
