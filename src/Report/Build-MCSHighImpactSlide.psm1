function Build-MCSHighImpactSlide {
    Param($Presentation,$CustomerName,$HighRecommendation)
    ...
    $Counter = 1
    $RecomNumber = 1
    foreach ($High in $HighRecommendation) {
        $RecomName = $High.Recommendation
        $Implication = $High.Implication
        Write-Debug ((get-date -Format 'yyyy-MM-dd HH:mm:ss') + ' - Editing Slide 10 - Adding Recommendation: ' + $RecomName)
        if ($Counter -lt 9) {
            ...
            Write-Debug ((get-date -Format 'yyyy-MM-dd HH:mm:ss') + ' - Editing Slide 10 - Adding new Slide..')
            $Counter = 1
            ...
            Write-Debug ((get-date -Format 'yyyy-MM-dd HH:mm:ss') + ' - Editing Slide 10 - Cleaning table of new slide..')
            $rowTemp = 2
            while ($rowTemp -lt 10) {
                $cell = 1
                while ($cell -lt 5) {
